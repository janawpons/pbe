...
class Rfid
    ...
    def wait_for_card
        ...
        @context.wait_for_status_change(queries) # espera fins que es detecti un canvi d'estat
        queries.ack_changes
    end

    def read_uid
        inicialitza
        puts 'Col·loca la targeta'
        puts
        wait_for_card
        
        puts 'Espera... llegint targeta'
        puts
        sleep(4)
        ...
        begin
            ...
            resposta = targeta.transmit("\xFF\xCA\x00\x00\x04") #llegeix l'uid de la targeta
            
            uid = resposta.unpack('H*').first.upcase # conversió a hexadecimal i majúscules
            
            puts "Ja pots retirar la targeta..."
            puts
            #m'he d'esperar a que la retiri?
            sleep(2)
